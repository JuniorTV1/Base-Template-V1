		...
		for k, v in pairs(Characters) do
			...
			if Characters[k].disabled then
				...
				option[#option + 1] = {
					...
					arrow = true,
					metadata = {
						{label = 'Prénom + Nom', value = v.firstname .. ' ' .. v.lastname},
						{label = 'Métier', value = v.job .. ' -> ' ..v.job_grade},
						{label = 'Date de naissance', value = v.dateofbirth},
						{label = 'Argent liquide', value = v.money},
						{label = 'Argent banque', value = v.bank},
						{label = 'Genre', value = v.sex == 'm' and TranslateCap('female') or TranslateCap('male')}
					  },
					onSelect = function(_, SelectedCharacter)
						SelectedCharacter = v.id
						CharacterOptions(Characters, slots, SelectedCharacter)
